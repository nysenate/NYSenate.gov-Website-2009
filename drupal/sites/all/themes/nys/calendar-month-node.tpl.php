...
<?php
  $monthviewClass = 'monthview calendar';
  if(isset($fields['node_data_field_date_field_public_hearing_value']['data']) && $fields['node_data_field_date_field_public_hearing_value']['data'] == 'Session') {
    $monthviewClass .= '-sessionday';
  }
  $event_type = str_replace(' ', '-', strtolower(theme('nyss_event_type', $node->nid)));
  if ($event_type) {
    $monthviewClass .= " event-type-$event_type";
  }
?>

<div class="view-item view-item-<?php print $view->name ?>">
  <div class="<?php print $monthviewClass; ?>" id="<?php print $node->date_id ?>">
    <?php print theme('calendar_stripe_stripe', $node); ?>
    <?php foreach ($fields as $field): if ($field['id'] != 'node-data-field-date-field-public-hearing-value'): ?>
      <div id="<?php print $field['id']; ?>" class="view-field view-data-<?php print $field['id']; if (isset($field['node_data_field_date_field_public_hearing_value']['data']) && $field['node_data_field_date_field_public_hearing_value']['data'] == 'Session') { print ' session-day'; }?>">
        <?php if ($field['label']): ?>
